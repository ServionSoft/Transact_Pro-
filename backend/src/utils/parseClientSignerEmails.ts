/** DocuSign / CRM use the same simple check as the legacy UI (no full RFC 5322). */
const SIMPLE_EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

const INVISIBLE_CHARS = /[\u200B-\u200D\uFEFF]/g;

/** Typos / garbage TLDs that pass SIMPLE_EMAIL_RE but are almost never valid (e.g. gmail.comd → tld `comd`). */
...
/** Full-domain typos that still end in a plausible-looking TLD. */
...
/**
 * Parses a user-entered field that may contain one or more addresses separated by comma, semicolon, or newline.
 * Returns lowercased, de-duplicated, validated emails in order (first = signer).
 */
...
/**
 * Stricter checks after SIMPLE_EMAIL_RE: length, suspicious domains/TLDs.
...
/** Validates every address in order; first is signer, rest are carbon copies. */
