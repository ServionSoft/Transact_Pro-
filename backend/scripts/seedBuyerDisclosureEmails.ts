/**
 * Idempotent seed: upsert buyer-disclosure email templates by template_key.
 *
 * Run:
 *   npm run db:seed:buyer-emails          # local + render when .env.render exists
 *   npm run db:seed:buyer-emails:local    # backend/.env only (matches npm run dev API)
 *   npm run db:seed:buyer-emails:render   # backend/.env.render only
 */
...
import pg from "pg";
import { BUYER_DISCLOSURE_EMAIL_TEMPLATES } from "../src/data/buyerDisclosureEmailTemplates.js";

const COMPASS_TASK_EMAIL_WIRES: { templateItemKey: string; emailTemplateKey: string }[] = [
  { templateItemKey: "compass_buyer:B09", emailTemplateKey: "missing_disclosure_request_la_tc" },
  { templateItemKey: "compass_buyer:B11", emailTemplateKey: "notes_questions_ba" },
  { templateItemKey: "compass_buyer:B13", emailTemplateKey: "missing_disclosure_request_la_tc" },
  { templateItemKey: "compass_buyer:B14", emailTemplateKey: "buyer_disclosures_separate_packet" },
  { templateItemKey: "compass_buyer:B15", emailTemplateKey: "buyer_disclosures_separate_additional" },
  { templateItemKey: "compass_buyer:B17", emailTemplateKey: "buyer_signed_docs_la_tc" },
  { templateItemKey: "compass_buyer:B18", emailTemplateKey: "buyer_signed_disclosures_la_tc" },
];
...
async function upsertTemplate(
  ...
  tpl: (typeof BUYER_DISCLOSURE_EMAIL_TEMPLATES)[number]
...
async function seedOneTarget(target: SeedTarget): Promise<void> {
  ...
  try {
    ...
    for (const tpl of BUYER_DISCLOSURE_EMAIL_TEMPLATES) {
