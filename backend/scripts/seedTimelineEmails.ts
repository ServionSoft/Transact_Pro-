/**
 * Idempotent seed: upsert timeline email templates by template_key.
 *
 * Run:
 *   npm run db:seed:timeline-emails          # local + render when .env.render exists
 *   npm run db:seed:timeline-emails:local    # backend/.env only
 *   npm run db:seed:timeline-emails:render   # backend/.env.render only
 */
...
import pg from "pg";
import { TIMELINE_EMAIL_TEMPLATES } from "../src/data/timelineEmailTemplates.js";

const COMPASS_TASK_EMAIL_WIRES: { templateItemKey: string; emailTemplateKey: string }[] = [
  { templateItemKey: "compass_listing:L23", emailTemplateKey: "timeline_parties" },
  { templateItemKey: "compass_listing:L24", emailTemplateKey: "timeline_email_seller" },
  { templateItemKey: "compass_buyer:B29", emailTemplateKey: "timeline_parties" },
  { templateItemKey: "compass_buyer:B30", emailTemplateKey: "timeline_client" },
];
...
async function upsertTemplate(
  ...
  tpl: (typeof TIMELINE_EMAIL_TEMPLATES)[number]
...
async function seedOneTarget(target: SeedTarget): Promise<void> {
  ...
  try {
    ...
    for (const tpl of TIMELINE_EMAIL_TEMPLATES) {
