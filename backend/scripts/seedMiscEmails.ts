/**
 * Idempotent seed: upsert misc email templates by template_key.
 *
 * Run:
 *   npm run db:seed:misc-emails          # local + render when .env.render exists
 *   npm run db:seed:misc-emails:local    # backend/.env only
 *   npm run db:seed:misc-emails:render   # backend/.env.render only
 */
...
import pg from "pg";
import { MISC_EMAIL_TEMPLATES } from "../src/data/miscEmailTemplates.js";

const COMPASS_TASK_EMAIL_WIRES: { templateItemKey: string; emailTemplateKey: string }[] = [
  { templateItemKey: "compass_listing:L43", emailTemplateKey: "vp_reminder" },
  { templateItemKey: "compass_listing:L44", emailTemplateKey: "closing_file_email" },
  { templateItemKey: "compass_buyer:B33", emailTemplateKey: "vp_reminder" },
  { templateItemKey: "compass_buyer:B34", emailTemplateKey: "closing_file_email" },
];
...
async function upsertTemplate(
  ...
  tpl: (typeof MISC_EMAIL_TEMPLATES)[number]
...
async function seedOneTarget(target: SeedTarget): Promise<void> {
  ...
  try {
    ...
    for (const tpl of MISC_EMAIL_TEMPLATES) {
