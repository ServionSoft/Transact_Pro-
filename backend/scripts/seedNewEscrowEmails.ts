/**
 * Idempotent seed: upsert new-escrow email templates by template_key.
 *
 * Run:
 *   npm run db:seed:new-escrow-emails          # local + render when .env.render exists
 *   npm run db:seed:new-escrow-emails:local    # backend/.env only
 *   npm run db:seed:new-escrow-emails:render   # backend/.env.render only
 */
...
import pg from "pg";
import { NEW_ESCROW_EMAIL_TEMPLATES } from "../src/data/newEscrowEmailTemplates.js";

const COMPASS_TASK_EMAIL_WIRES: { templateItemKey: string; emailTemplateKey: string }[] = [
  { templateItemKey: "compass_listing:L09", emailTemplateKey: "new_pre_escrow" },
];
...
async function upsertTemplate(
  ...
  tpl: (typeof NEW_ESCROW_EMAIL_TEMPLATES)[number]
...
async function seedOneTarget(target: SeedTarget): Promise<void> {
  ...
  try {
    ...
    for (const tpl of NEW_ESCROW_EMAIL_TEMPLATES) {
