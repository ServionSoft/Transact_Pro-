/**
 * Idempotent seed: upsert home-warranty email templates by template_key.
 *
 * Run:
 *   npm run db:seed:home-warranty-emails          # local + render when .env.render exists
 *   npm run db:seed:home-warranty-emails:local    # backend/.env only
 *   npm run db:seed:home-warranty-emails:render   # backend/.env.render only
 */
...
import pg from "pg";
import { HOME_WARRANTY_EMAIL_TEMPLATES } from "../src/data/homeWarrantyEmailTemplates.js";

const COMPASS_TASK_EMAIL_WIRES: { templateItemKey: string; emailTemplateKey: string }[] = [
  { templateItemKey: "compass_listing:L40", emailTemplateKey: "warranty_commission_compass" },
  { templateItemKey: "compass_buyer:B25", emailTemplateKey: "warranty_commission_compass" },
];
...
async function upsertTemplate(
  ...
  tpl: (typeof HOME_WARRANTY_EMAIL_TEMPLATES)[number]
...
async function seedOneTarget(target: SeedTarget): Promise<void> {
  ...
  try {
    ...
    for (const tpl of HOME_WARRANTY_EMAIL_TEMPLATES) {
