/**
 * Idempotent seed: upsert root-level email templates by template_key.
 *
 * Run:
 *   npm run db:seed:root-emails          # local + render when .env.render exists
 *   npm run db:seed:root-emails:local    # backend/.env only
 *   npm run db:seed:root-emails:render   # backend/.env.render only
 */
...
import pg from "pg";
import { ROOT_EMAIL_TEMPLATES } from "../src/data/rootEmailTemplates.js";
...
async function upsertTemplate(
  ...
  tpl: (typeof ROOT_EMAIL_TEMPLATES)[number]
...
async function seedOneTarget(target: SeedTarget): Promise<void> {
  ...
  try {
    ...
    for (const tpl of ROOT_EMAIL_TEMPLATES) {
